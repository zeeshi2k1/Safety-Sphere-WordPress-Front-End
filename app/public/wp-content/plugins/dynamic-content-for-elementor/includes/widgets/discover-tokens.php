...
class DiscoverTokens extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    protected function safe_register_controls()
    {
        ...
        $this->add_control('type', ['label' => esc_html__('Type', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'options' => ['acf' => 'acf', 'author' => 'author', 'date' => 'date', 'expr' => 'expr', 'jet' => 'jet', 'metabox' => 'metabox', 'option' => 'option', 'post' => 'post', 'product' => 'product', 'query' => 'query', 'system' => 'system', 'term' => 'term', 'user' => 'user', 'wp_query' => 'wp_query'], 'default' => 'post']);
        $this->add_control('notice_resource_intensive', ['type' => Controls_Manager::RAW_HTML, 'raw' => esc_html__('This token can be resource intensive as it queries the database and the speed depends on how many elements you have on your site', 'dynamic-content-for-elementor'), 'content_classes' => 'elementor-panel-alert elementor-panel-alert-warning', 'condition' => ['type' => 'query']]);
    ...
    protected function safe_render()
    {
        ...
        if (!Tokens::is_active()) {
            Helper::notice(\false, esc_html__('Tokens are not active. You can activate it from WP Dashboard > Dynamic Content for Elementor > Settings > Tokens', 'dynamic-content-for-elementor'));
            return;
        }
        Helper::notice(\false, esc_html__('This widget is only visible to administrators in order to better understand the tokens syntax, so you can put it on any public page without problem.', 'dynamic-content-for-elementor'));
    ...
    protected function start_table()
    {
        echo '<table>';
        echo '<tr>';
        echo '<th>' . esc_html__('What', 'dynamic-content-for-elementor') . '</th>';
        echo '<th>' . esc_html__('Token', 'dynamic-content-for-elementor') . '</th>';
        echo '<th>' . esc_html__('Result', 'dynamic-content-for-elementor') . '</th>';
        echo '</tr>';
    }
    /**
     * End the table
    ...
    protected function check_plugin_dependencies(string $type)
    {
        ...
        if (!empty($plugin_depends) && !Helper::check_plugin_dependencies(\false, $plugin_depends)) {
            ...
            /* translators: %1$s: list of plugin names */
            $message = \sprintf(esc_html__('You need %1$s plugin to use this token', 'dynamic-content-for-elementor'), $plugins);
    ...
    protected function get_demo()
    {
        return [
            ...
            'acf' => [
                ...
                'description' => esc_html__('This token is useful for retrieving Advanced Custom Fields', 'dynamic-content-for-elementor'),
                // Plugin dependencies
                'plugin_depends' => ['acf'],
            ...
            'author' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving the author fields of the current page. An author is the user who created the current post or page', 'dynamic-content-for-elementor'),
            ...
            'jet' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving JetEngine Fields', 'dynamic-content-for-elementor'),
                // Plugin dependencies
            ...
            'metabox' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving Meta Box Fields', 'dynamic-content-for-elementor'),
                // Plugin dependencies
            ...
            'post' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving the fields of the current page', 'dynamic-content-for-elementor'),
                // Static fields to show
            ...
            'expr' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful to make expressions. It supports nested Tokens', 'dynamic-content-for-elementor'),
                // Static fields to show
            ...
            'user' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving the fields of the current user', 'dynamic-content-for-elementor'),
                // Static fields to show
            ...
            'product' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving the fields of the current WooCommerce product', 'dynamic-content-for-elementor'),
                // Plugin dependencies
            ...
            'query' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for to generate a query on your posts, pages, users, and taxonomies', 'dynamic-content-for-elementor'),
                // Static fields to show
            ...
            'system' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful to fetch the current request parameters and data', 'dynamic-content-for-elementor'),
                // Fields to hide when executed 'functions'
                'hidden' => ['post:actions'],
                // Function to sanitize all fields
            ...
            'date' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful when you need to work with date', 'dynamic-content-for-elementor'),
                // Static fields to show
            ...
            'term' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving the fields of the current term on a Term Archive or the terms on posts', 'dynamic-content-for-elementor'),
                // Static fields to show
            ...
            'wp_query' => [
                // Description to show before the table
                'description' => esc_html__('This token is useful for retrieving the query vars of the current WP_Query', 'dynamic-content-for-elementor'),
                // Functions to populate fields
