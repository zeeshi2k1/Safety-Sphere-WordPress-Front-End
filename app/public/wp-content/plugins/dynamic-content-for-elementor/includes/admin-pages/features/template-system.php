...
class TemplateSystem
{
    ...
    {
        return esc_html__('Template System (Deprecated)', 'dynamic-content-for-elementor');
    }
    /**
    ...
    public function render()
    {
        ...
        $templates = array(esc_html__('None', 'dynamic-content-for-elementor'));
        ...
        $preview = array();
        $dceTemplate = array();
        $dceTemplate['post-types']['label'] = esc_html__('Post Types', 'dynamic-content-for-elementor');
        // Types
        ...
        foreach ($typesRegistered as $chiave) {
            ...
            $dceTemplate['post-types']['templates'][$chiave]['single'] = esc_html__('Single', 'dynamic-content-for-elementor');
            $dceTemplate['post-types']['templates'][$chiave]['archive'] = esc_html__('Archive', 'dynamic-content-for-elementor');
        }
        // Taxonomy
        ...
        $dceTemplate['taxonomies']['label'] = esc_html__('Taxonomies', 'dynamic-content-for-elementor');
        foreach ($customTaxonomies as $chiave) {
            ...
            $dceTemplate['taxonomies']['templates'][$chiave]['single'] = esc_html__('Single', 'dynamic-content-for-elementor');
            $dceTemplate['taxonomies']['templates'][$chiave]['archive'] = esc_html__('Archive', 'dynamic-content-for-elementor');
        }
        $dceTemplate['other-pages']['label'] = esc_html__('Other Pages', 'dynamic-content-for-elementor');
        // Search
        $chiave = 'search';
        $preview[$chiave] = get_search_link('lorem ipsum');
        $dceTemplate['other-pages']['options'][$chiave] = esc_html__('Search', 'dynamic-content-for-elementor');
        $dceTemplate['other-pages']['templates'][$chiave]['archive'] = esc_html__('Archive', 'dynamic-content-for-elementor');
        // User
        $chiave = 'user';
        $preview[$chiave] = get_author_posts_url(get_current_user_id());
        $dceTemplate['other-pages']['options'][$chiave] = esc_html__('User', 'dynamic-content-for-elementor');
        $dceTemplate['other-pages']['templates'][$chiave]['archive'] = esc_html__('Archive', 'dynamic-content-for-elementor');
        ?>
        ...
        foreach ($dceTemplate as $tkey => $tvalue) {
            foreach ($tvalue['options'] as $chiave => $label_t) {
                ...
                foreach ($tvalue['templates'][$chiave] as $skey => $svalue) {
                    ...
                    } elseif (isset($this->options[$dce_key]) && $this->options[$dce_key]) {
    ...
    private function archive_settings($dce_key)
    {
        ...
        $columns = array('md' => esc_html__('Desktop', 'dynamic-content-for-elementor'), 'sm' => esc_html__('Tablet', 'dynamic-content-for-elementor'), 'xs' => esc_html__('Mobile', 'dynamic-content-for-elementor'));
