jQuery(window).on("elementor/frontend/init", () => {
	class OsmMap extends elementorModules.frontend.handlers.Base {
		...
		onInit() {
			...
			this.getCoordFromAdress(elementSettings.address, (lon, lat) => {
				this.map = L.map(this.elements.$mapWrapper[0]).setView(
					{ lon: lon, lat: lat },
					elementSettings.zoom.size,
				);
		...
		onElementChange(propertyName) {
			...
			if (propertyName === "address") {
				this.getCoordFromAdress(
					this.getElementSettings("address"),
					(lon, lat) => {
						this.map.removeLayer(this.marker); // remove previous marker.
						this.map.setView(
							{ lon: lon, lat: lat },
							this.getElementSettings("zoom").size,
						);
						this.marker = L.marker({ lon: lon, lat: lat });
						this.marker.addTo(this.map);
					},
	...
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/dce-osm-map.default",
		addHandler,
	);
});
