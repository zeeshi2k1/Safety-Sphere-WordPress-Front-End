...
class TemplateSystem
{
    ...
    {
        $columns['dce_template'] = DCE_PRODUCT_NAME . ' - ' . esc_html__('Template System', 'dynamic-content-for-elementor');
        return $columns;
    }
    /**
    ...
    private function get_template_links($template_id)
    {
        ...
        return '<a href="' . get_permalink($template_id) . '" target="blank">' . wp_kses_post(get_the_title($template_id)) . '</a> - ' . '<a href="' . admin_url('post.php?post=' . $template_id . '&action=edit') . '" target="blank">' . esc_html__('Edit', 'dynamic-content-for-elementor') . '</a>';
    }
    /**
    ...
    private function get_taxonomy_column_content($term_id, $template_name)
    {
        ...
        return '<b>' . \strtoupper($template_name) . '</b> ' . $this->get_template_links($template_id) . '<br>';
    }
    /**
    ...
    {
        $columns['dce_template'] = DCE_BRAND . ' ' . esc_html__('Template', 'dynamic-content-for-elementor');
        return $columns;
    }
    /**
    ...
    public function get_template($template_id, $inline_css = \false)
    {
        ...
        if ($doc && $doc->is_built_with_elementor()) {
            ...
            if ($doc->get_name() === 'loop-item') {
                ...
                $loop_css = $this->print_loop_document_css($template_id);
            }
