...
trait Wp
{
    ...
    {
        $args = ['sort_order' => 'desc', 'sort_column' => 'menu_order', 'hierarchical' => \true, 'post_type' => 'page', 'post_status' => 'publish'];
    ...
    public static function get_roles($everyone = \false, $remove_admin = \false)
    {
        ...
        $ret = array();
        if ($everyone) {
            $ret['everyone'] = esc_html__('Everyone', 'dynamic-content-for-elementor');
        }
    ...
    public static function get_translated_post_id($original_id, $post_type = \false)
    {
        ...
        $translated_id = apply_filters('wpml_object_id', $original_id, $post_type, \true);
        if (!$translated_id) {
            return $original_id;
        }
    ...
    {
        if (!\DynamicContentForElementor\Tokens::is_active() || !\DynamicContentForElementor\Helper::can_register_unsafe_controls()) {
